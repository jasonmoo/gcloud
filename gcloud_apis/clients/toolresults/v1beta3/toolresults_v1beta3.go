...
type Any struct {
	...
	// avoid any lookup. Therefore, binary compatibility need to be
...
type FileReference struct {
	...
	// example: http://storage.googleapis.com/mybucket/path/to/test.xml
	//
	// An
...
type Status struct {
	...
	// English. The user- facing error message should be localized and
	// stored in the [google.rpc.Status.details][] field.
...
type Step struct {
	...
	// Description: A description of this tool For example: mvm clean
	...
	// this execution which already has the same name and
	// dimension_value.
	//
...
type TestSuiteOverview struct {
	...
	//
	// - In
	// response: always set - In create/update request: never
	...
	//
	// - In response: always set - In create/update
	// request: never
	...
	//
	// - In response: optional - In create/update
	// request: never
...
// FilterByDisplayName sets the optional parameter
// "filterByDisplayName": If set, only return histories with the given
// name.
func (c *ProjectsHistoriesListCall) FilterByDisplayName(filterByDisplayName string) *ProjectsHistoriesListCall {
	c.opt_["filterByDisplayName"] = filterByDisplayName
	return c
}
...
func (c *ProjectsHistoriesListCall) Do() (*ListHistoriesResponse, error) {
	...
	params := make(url.Values)
	params.Set("alt", "json")
	if v, ok := c.opt_["filterByDisplayName"]; ok {
		params.Set("filterByDisplayName", fmt.Sprintf("%v", v))
	}
